...
router.post('/',
  authorize('PROPOSAL_MANAGER', 'SOLUTION_ARCHITECT', 'ADMIN', 'CO_ADMIN'),
  rfpController.createRFP
);

// PATCH update RFP
router.patch('/:id',
  authorize('PROPOSAL_MANAGER', 'SOLUTION_ARCHITECT', 'BID_REVIEWER', 'ADMIN', 'CO_ADMIN'),
  rfpController.updateRFP
);

// DELETE RFP (Proposal Managers only)
router.delete('/:id',
  authorize('PROPOSAL_MANAGER', 'ADMIN', 'CO_ADMIN'),
