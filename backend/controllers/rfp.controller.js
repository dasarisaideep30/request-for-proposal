...
const updateRFP = async (req, res) => {
  try {
    ...
    // Admin Override Logic: If user is rejecting or admin is approving
    if (updateData.status === 'REJECTED' && req.user.role !== 'ADMIN') {
      // Logic for user rejection: Notify Admin
      console.log(`[RFP] ${currentRFP.rfpNumber} rejected by ${req.user.firstName}. Routing to Admin for final decision.`);
      // We still update it to REJECTED, but admin will see it in their dashboard
    }
